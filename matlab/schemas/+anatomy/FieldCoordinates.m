...
classdef FieldCoordinates < dj.Imported
    ...
    methods (Static)
        function [x,y,rot,scale,go] = alignImages(image1,image2,varargin)
            ...
            %
            %
            %             ref_map = normalize(vessels*2.^2)*0.5;
            %             ref_map = ref_map*50;
            %
            %             %# coordinates
            %             [X,Y] = meshgrid(1:size(ref_map,2), 1:size(ref_map,1));
            %             Z = zeros(size(ref_map,1),size(ref_map,2));
            %             Xidx = X - size(ref_map,2)/2;
            %             Yidx = Y - size(ref_map,1)/2;
            %
            %             %# plot each slice as a texture-mapped surface (stacked along the Z-dimension)
            %             surface('XData',(Xidx)/scale, 'YData',(Yidx)/scale, 'ZData',Z/scale, ...
            %                 'CData',flipud(ref_map), 'CDataMapping','direct', ...
            %                 'EdgeColor','none', 'FaceColor','texturemap',...
            %                 'AlphaData',flipud(abs(ref_map-max(ref_map(:)))*2),'FaceAlpha','texturemap','alphadatamapping','direct');
            %
            %
            %             imS = anatomy.FieldCoordinates.processImage(im2,'exp',0.7);
            %             imS2 = imS*50+50;
            %
            %             %# coordinates
            %             [X,Y] = meshgrid(1:size(imS2,2), 1:size(imS2,1));
            %             Z = zeros(size(imS2,1),size(imS2,2));
            %             Xidx = X - size(imS2,2)/2;
            %             Yidx = Y - size(imS2,1)/2;
            %
            %             %# plot each slice as a texture-mapped surface (stacked along the Z-dimension)
            %             h = surface('XData',(Xidx), 'YData',(Yidx), 'ZData',Z, ...
            %                 'CData',flipud(imS2), 'CDataMapping','direct', ...
            %                 'EdgeColor','none', 'FaceColor','texturemap',...
            %                 'AlphaData',flipud(imS*100),'FaceAlpha','texturemap','alphadatamapping','direct');
            %
            %
            % %             axis image
            %             cmap = bsxfun(@times,[1 0 0],[0:0.02:1]');
