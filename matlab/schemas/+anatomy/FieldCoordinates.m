        ...
        function alignScan(self,keyI, varargin)
            ...
            if length(frames)>1
                ...
                for islice =length(frames):-1:1
                    ...
                    im(round(y_pos(islice)+1):round(y_pos(islice))+size(frame,1), ...
                        round(x_pos(islice)+1):round(x_pos(islice))+size(frame,2)) = ...
...
%                 out_im = hsv2rgb(im(:,:,[3 2 1]));
        ...
        function fmask = filterMask(self, ref_mask)
            ...
            fmask = fmask(...
                round(size(fmask,1)/2 - sz(1)/2 + 1):round(size(fmask,1)/2 + sz(1)/2),...
                round(size(fmask,2)/2 - sz(2)/2 + 1):round(size(fmask,2)/2 + sz(2)/2));
        end
    end
