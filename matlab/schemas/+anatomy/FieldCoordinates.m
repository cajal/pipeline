...
classdef FieldCoordinates < dj.Manual
    methods
        function alignScan(self,keyI, varargin)
            ...
            if length(frames)>1
                ...
                for islice =length(frames):-1:1
                    frame = self.filterImage(anatomy.FieldCoordinates.normalize(frames{islice}),self.createTform(tfp));
                ...
                im = anatomy.FieldCoordinates.normalize(frames{1}.^params.contrast);
                x_pos = 0;
                y_pos = 0;
            end
            ...
            [x_offset, y_offset, rotation, tfp.scale, go] = ...
                self.alignImages(anatomy.FieldCoordinates.normalize(ref_map),anatomy.FieldCoordinates.normalize(im),...
        ...
        function out_im = plot(self,varargin)
            ...
            %  construct image
            ref_map = anatomy.FieldCoordinates.normalize(ref_map.^params.exp);
            if params.inv; ref_map = 1-ref_map;end
            idxX = [];idxY = []; frame = [];
            for islice = 1:length(frames)
                x_offset = x(islice);
                y_offset = y(islice);
                imS = self.filterImage(anatomy.FieldCoordinates.normalize(frames{islice}),tforms{islice});
            ...
            im(:,:,2) = anatomy.FieldCoordinates.normalize(im(:,:,2));
        ...
        function plot3D(self)
            ...
            % plot
            figure
            ref_map = anatomy.FieldCoordinates.normalize(ref_map{1}*2.^2)*0.5;
            ref_map = ref_map*50;
        ...
        function fmask = filterMask(self, ref_mask)
            ...
            sz = size(frame);
            imS = self.filterImage(anatomy.FieldCoordinates.normalize(frame),tform);            % apply rotation/flips 
            ...
            fmask = self.filterImage(anatomy.FieldCoordinates.normalize(fmask),tform,1)>0;
            fmask = fmask(...
    ...
    methods (Static)
        function [x,y,rot,scale,go] = alignImages(image1,image2,varargin)
            ...
            % assign images
            vessels = imresize(anatomy.FieldCoordinates.normalize(image1),params.resize);
            im1 = vessels;
            im2 = imresize(anatomy.FieldCoordinates.normalize(image2),params.resize);
        ...
        function im = processImage(im,varargin)
            ...
            if params.exp
                im = anatomy.FieldCoordinates.normalize(im).^params.exp;
            end
            
            if params.normalize
                im = anatomy.FieldCoordinates.normalize(im);
            end
