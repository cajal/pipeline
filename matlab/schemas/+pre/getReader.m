...
if nargin <= 1 || ~exist(cache_dir, 'file')==7
    ...
    try
        fprintf('Loading from %s\n', cache_path);
        tpath = getLocalPath(fullfile(cache_path, sprintf('%s_*%03u_*.tif', basename, scanIdx)));
        reader = ne7.scanimage.Reader4(tpath);
    catch
        from = fullfile(getLocalPath(path), sprintf('%s_*%03u_*.tif', basename, scanIdx));
        fprintf('Copying %s to %s\n', from, cache_path);
        copyfile(from, cache_path)
        path = getLocalPath(fullfile(cache_path, sprintf('%s_*%03u_*.tif', basename, scanIdx)));
        reader = ne7.scanimage.Reader4(path);
    end
end