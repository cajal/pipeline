...
def plot_fitting(key, start, end):

    if 'tracking_method' not in key.keys():
        raise KeyError
    
    avi_path = (Eye & key).get_video_path()
    center, radius = (pupil.FittedPupil.Circle & key & 'frame_id > {}'.format(start) & 'frame_id < {}'.format(end+1)).fetch(
    ...
    plt.subplots_adjust(left=0, bottom=0, right=1,
                        top=1, wspace=0, hspace=0)
    plt.xlim(0, cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    plt.ylim(0, cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

    plt.gca().invert_yaxis()

    if key['tracking_method'] == 1:
        # manual
        contours = (pupil.Tracking.ManualTracking & key & 'frame_id > {}'.format(start) & 'frame_id < {}'.format(end+1)).fetch(
        ...
        for frame_num in range(start, end):
            ...
            if contours[frame_num-start] is not None:
                ind = frame_num-start
                plt_circle = plt.Circle(center[ind], radius[ind], color='b', fill=False)
                ax.add_artist(plt_circle)
                ax_scatter = ax.scatter(contours[ind].squeeze()[:,0], contours[ind].squeeze()[:,1], s=4**2,
                                    color='red', alpha=.5)
        ...
        for frame_num in range(start, end):
            ...
            if center[ind] is not None:
                ind = frame_num-start
                center[ind][0] = cropped_coords[1]
                center[ind][1] = cropped_coords[3]