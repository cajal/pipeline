...
for pipe in [reso, meso]:
    ...
    pipe.RasterCorrection().populate(next_scans, reserve_jobs=True,
                                     suppress_errors=True)
    pipe.MotionCorrection().populate(next_scans, reserve_jobs=True,
                                     suppress_errors=True)
    ...
    pipe.Activity().populate(next_scans, {'spike_method': 5}, reserve_jobs=True,
                             suppress_errors=True)
    full_scans = (pipe.ScanInfo().proj() & pipe.Activity()) - (
                pipe.ScanInfo.Field() - pipe.Activity())
    pipe.ScanDone().populate(next_scans & full_scans, reserve_jobs=True,
                             suppress_errors=True)
